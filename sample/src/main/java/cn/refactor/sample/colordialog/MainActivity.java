...
public class MainActivity extends AppCompatActivity {
    ...
    private void showPromptDlg() {
        new PromptDialog(this).setDialogType(PromptDialog.DialogType.DIALOG_TYPE_INFO).setAnimationEnable(true).setTitleText(getString(R.string.info))
                              .setContentText(getString(R.string.text_data)).setPositiveListener(getString(R.string.ok), new PromptDialog.OnPositiveListener() {
            @Override public void onClick(PromptDialog dialog) {
                dialog.setContentText("teste suceaseseaaaaaaaaasuceaseseaaaaaaaaaaaaaaaaaaaaaaa" +
                                              "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaassosuceaseseaaaaaaaaaaaaaaaaaaaaaa" +
                                              "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" +
                                              "aaassoaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" +
                                              "aaaaaaaaaaaaaaaaasso").setPositiveListener(new PromptDialog.OnPositiveListener() {
                    @Override public void onClick(PromptDialog dialog) {
                        Toast.makeText(MainActivity.this, "asease", Toast.LENGTH_SHORT).show();
                        dialog.dismiss();
                    }
                }).setTitleText(getString(R.string.success)).changeAlertType(PromptDialog.DialogType.DIALOG_TYPE_SUCCESS);
            }
        }).setNegativeListener(getString(R.string.cancel), new PromptDialog.OnNegativeListener() {
            @Override public void onClick(PromptDialog dialog) {
                dialog.dismiss();
            }
        }).show();
    ...
    public void showTextDialog(View view) {
        ...
        dialog.setPositiveListener(getString(R.string.text_iknow), new ColorDialog.OnPositiveListener() {
            @Override public void onClick(ColorDialog dialog) {
